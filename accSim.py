...
while (hand1[0] not in cards or hand2[0] not in cards or hand1[1] not in cards or hand2[1] not in cards):
    ff = input("First card of first hand: ")
    hand1[0] = [conv(ff[0]), ff[1]]
    sf = input("Second card of first hand: ")
    hand1[1] = [conv(sf[0]), sf[1]]

    fs = input("First card of second hand: ")
    hand2[0] = [conv(fs[0]), fs[1]]
    ss = input("Second card of second hand: ")
    hand2[1] = [conv(ss[0]), ss[1]]
...
playerHands.append(hand1[0])
cardsInDeck -= 1
cards.remove(hand1[0])
playerHands.append(hand1[1])
cardsInDeck -= 1
cards.remove(hand1[1])
playerHands.append(hand2[0])
cardsInDeck -= 1
cards.remove(hand2[0])
playerHands.append(hand2[1])
cardsInDeck -= 1
cards.remove(hand2[1])
...
for board in list(combinations(cards, 5)):
    if (count % 1000 == 0):
        # print(count)
        pass
    count+=1
    ...
    for i in range(int(numPlayers)):
        # for testing
        #board = [[13, "s"], [3, "c"], [3, "h"], [14, "d"], [14, "c"]]
        #
        sevenCards = list(board)
    ...
    if (1 in winningPlayers and 2 in winningPlayers):
        winArr[2] += 1
    elif (1 in winningPlayers):
        winArr[0] += 1
    else:
        winArr[1] += 1

print("\n[Hand 1 wins, Hand 2 wins, Chops]: ", winArr)
print("Frequencies: ", [str(freq * 100 / float(len(list(combinations(cards, 5)))))+"%" for freq in winArr])
print(royals)